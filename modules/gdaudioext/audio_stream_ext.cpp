...
int AudioStreamPlaybackExt::_mix_internal(AudioFrame *p_buffer, int p_frames) {
    ...
    while(pos < p_frames) {
        if(frame_read_pos >= frame_read_len) {
            ...
            ERR_FAIL_COND_V(error < 0, pos);
            
            error = avcodec_receive_frame(base->codec_context, base->frame);
            ERR_FAIL_COND_V(error < 0, pos);
            
            int alloc_size = av_samples_get_buffer_size(nullptr, base->stream->codecpar->ch_layout.nb_channels, base->frame->nb_samples, AudioStreamExt::DESTINATION_FORMAT, 0);
            frame_read_buffer = (float *)memalloc(alloc_size);
            frame_read_pos = 0;
            frame_read_len = base->frame->nb_samples * base->stream->codecpar->ch_layout.nb_channels;
...
void AudioStreamPlaybackExt::start(double p_from_pos) {
	active = true;
	seek(p_from_pos);
	begin_resample();
}
...
double AudioStreamPlaybackExt::get_playback_position() const {
	return last_position;
}

void AudioStreamPlaybackExt::seek(double p_time) {
	if (!active) {
		return;
	}
...
Ref<AudioStreamPlayback> AudioStreamExt::instantiate_playback() {
	Ref<AudioStreamPlaybackExt> playback;
	
	ERR_FAIL_COND_V_MSG(source.is_empty(), playback, "No source specified. Please call the 'create' method.");
...
void AudioStreamExt::_run_load_job(void *p_self) {
	...
	av_opt_set_int(self->swr, "in_channel_count", self->codec_context->ch_layout.nb_channels, 0);
	av_opt_set_int(self->swr, "out_channel_count", self->codec_context->ch_layout.nb_channels, 0);
	av_opt_set_int(self->swr, "in_channel_layout", self->codec_context->ch_layout.u.mask, 0);
	av_opt_set_int(self->swr, "out_channel_layout", self->codec_context->ch_layout.u.mask, 0);
...
void AudioStreamExt::create(String p_source) {
	ERR_FAIL_COND_MSG(!source.is_empty(), "Stream has already been created.");
	ERR_FAIL_COND_MSG(p_source.is_empty(), "Invalid source specified.");
